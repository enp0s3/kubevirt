...
// projectParameters defines the operator and the sub-operator names
var projectParameters = map[string]string{
	"operator":     "kubevirt",
	"sub_operator": "kubevirt",
}

func CustomLinterRules(problems []promlint.Problem, mf *dto.MetricFamily) []promlint.Problem {
	// Check metric prefix
	operator := projectParameters["operator"]
	subOperator := projectParameters["sub_operator"]
	nameParts := strings.Split(*mf.Name, "_")

	if nameParts[0] != operator {
		problems = append(problems, promlint.Problem{
			Metric: *mf.Name,
			Text:   fmt.Sprintf("name need to start with '%s_'", operator),
		})
	} else if operator != subOperator && nameParts[1] != subOperator {
		problems = append(problems, promlint.Problem{
			Metric: *mf.Name,
			Text:   fmt.Sprintf("name need to start with \"%s_%s_\"", operator, subOperator),
		})
	}
